...
$sql = "SELECT * FROM sem1,register where sem1.reg_no=register.reg_no order by sem1.sem1obtained desc";
$result = mysqli_query($con,$sql)or die(mysqli_error($con));
$i=1;
echo "<h2 style='color:white;text-align:center;'>SEMESTER ONE REPORT RANKING WISE</h2>";
...
while($row = mysqli_fetch_array($result)) {
    $regno = $row['reg_no'];
    $s1ob = $row['sem1obtained'];
    $s1back = $row['sem1backs'];
