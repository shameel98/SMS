...
$sql = "SELECT * FROM sem2,register where sem2.reg_no=register.reg_no order by sem2.sem2obtained desc";
$result = mysqli_query($con,$sql)or die(mysqli_error($con));
$i=1;
echo "<h2 style='color:white;text-align:center;'>SEMESTER TWO REPORT RANKING WISE</h2>";
...
while($row = mysqli_fetch_array($result)) {
    $regno = $row['reg_no'];
    $s1ob = $row['sem2obtained'];
    $s1back = $row['sem2backs'];
