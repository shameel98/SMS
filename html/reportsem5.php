...
$sql = "SELECT * FROM sem5,register where sem5.reg_no=register.reg_no order by sem5.sem5obtained desc";
$result = mysqli_query($con,$sql)or die(mysqli_error($con));
$i=1;
echo "<h2 style='color:white;text-align:center;'>SEMESTER FIVE REPORT RANKING WISE</h2>";
...
while($row = mysqli_fetch_array($result)) {
    $regno = $row['reg_no'];
    $s1ob = $row['sem5obtained'];
    $s1back = $row['sem5backs'];
